...
public class MapWidget extends AbstractWidget {
    ...
    public boolean mouseClicked(double mouseX, double mouseY, int button) {
        if (button == 1) {
            // Right click - show create waypoint button in parent screen
            mapScreen.showCreateWaypointButton(mouseX, mouseY);
            return true;
        }
    ...
    protected void onDrag(double mouseX, double mouseY, double dragX, double dragY) {
        posX -= dragX;
        posY += dragY;
        // Hide create waypoint button when dragging the map
        mapScreen.hideCreateWaypointButton();
    }
